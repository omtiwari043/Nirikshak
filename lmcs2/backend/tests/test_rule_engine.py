...
def test_bikano_style_ocr_values_are_not_reported_missing():
    text = """
    NET WEIGHT 200g
    MAX.RETAIL PRICE55.00 (INCL.OF ALL TAXES)
    DATE 0F MANUFACTURE:20/04/2024
    """
    result = evaluate_compliance(text, False, "physical_package", [])
    found_codes = {item["code"] for item in result["declarations_found"]}
    violation_codes = {item["declaration_code"] for item in result["violations"]}
    assert {"NET_QUANTITY", "MRP", "MFG_DATE"} <= found_codes
    assert not {"NET_QUANTITY", "MRP", "MFG_DATE"} & violation_codes
    assert result["structured_values"]["NET_QUANTITY"] == "200g"
    assert "55.00" in result["structured_values"]["MRP"]
    assert result["structured_values"]["MFG_DATE"] == "20/04/2024"
