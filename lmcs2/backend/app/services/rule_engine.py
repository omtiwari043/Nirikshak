...
def _extract_structured_values(text: str) -> dict[str, str]:
    ...
    # OCR commonly preserves the price but drops its currency prefix.  This
    # must not turn a visible MRP declaration into a false "missing" finding.
    mrp = re.search(
        rf"(?:m\s*\.?\s*r\s*\.?\s*p|max\.?\s*retail\s+price|maximum\s+retail\s+price)"
        rf".{{0,100}}?({money}|\d+\.\d{{1,2}})",
        normalized,
        re.I,
    )
    if mrp:
    ...
    quantity = re.search(r"(?:net\s*(?:quantity|qty|weight)\s*[:=-]?\s*).{0,80}?(\d+(?:\.\d+)?\s*(?:kg|g|gm|mg|ml|l|litre|liter|n|units?|pieces?|pcs))\b", normalized, re.I)
    if quantity:
        values["NET_QUANTITY"] = quantity.group(1)
    date = re.search(r"(?:mfd|mfg|manufactured|packed\s+on|date\s*(?:of|0f)\s+(?:manufacture|packing))[^\n]{0,100}?((?:0?[1-9]|[12]\d|3[01])[/-](?:0?[1-9]|1[0-2])[/-](?:19\d{2}|20\d{2}|\d{2})|(?:0[1-9]|1[0-2])[/-](?:19|20)\d{2}|(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*\.?\s*(?:19|20)\d{2})", normalized, re.I)
    if date:
